...
LongNum LongNum::operator/(const LongNum &other) const {
    ...
    while (!divisor.frac_part.empty()) {
        if (!dividend.frac_part.empty()) {
    ...
    // Приведение делимого к целому числу
    while (!dividend.frac_part.empty()) {
        dividend.int_part.insert(dividend.int_part.begin(), dividend.frac_part[0]);
        dividend.frac_part.erase(dividend.frac_part.begin());
        divisor.int_part.insert(divisor.int_part.begin(), 0);
    }

    // Удаление незначащих нулей (типа 000123.123)
    // divisor
    ...
    // Инициализация промежуточного остатка (должен быть пустым)
    LongNum remainder;
    ...
    while (remainder.int_part.size() > 1 && remainder.int_part.back() == 0) {
        remainder.int_part.pop_back();
    }
